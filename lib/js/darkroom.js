...
;(function(window, document, fabric) {
  ...
  Darkroom.util = {
    ...
    forEachValue: function(obj, cb) {

      Object.keys(obj).forEach(function(prop) { cb(obj[prop]); });
    },

    cloneImage: function(image, cb) {
    ...
    makeStatic: function(fabricObject) {
  ...
  Darkroom.prototype = {
    ...
    defaults: {
      ...
      // HACK: breaks history plugin, so we disable it in this.initPlugins
      imageUrl: null,
    ...
    init: function(element, options, plugins) {
      ...
      if (imageUrl) { image.src = element.src = imageUrl; }
      else          { image.src = element.src;            }
    },
    ...
    initPlugins: function(plugins) {
      ...
      // HACK: imageUrl option breaks history plugin, so disable it
      if (this.options.imageUrl) { this.options.plugins.history = false}
      ...
      for (var name in plugins) {
        ...
        if (options !== false && plugins.hasOwnProperty(name)) {

          try               { this.plugins[name] = new plugin(this, options);                }
          catch (exception) { console.log('Darkroom plugin ' + name + ' failed', exception); }
        }
      }
    },
    ...
    replaceImage: function(newFabricImage, changedEvent) {

      var currentFabricImage = this.image,
          darkroom           = this,
          options            = this.options,
          canvas             = this.canvas;

      var height = this.getHeight(newFabricImage.getHeight()),
          width  = this.getWidth(newFabricImage.getWidth());

      Darkroom.util
        .makeStatic(newFabricImage)
        .set({width: width, height: height});

      canvas.remove(currentFabricImage);
      ...
      newFabricImage.moveTo(0);
      darkroom.image = newFabricImage;
      if (changedEvent) { darkroom.dispatchEvent('image:change'); }
    },

    getHeight: function(height) {
      var options = this.options;
      return Math.min(Math.max(options.minHeight, height), options.maxHeight);
    },

    getWidth : function(width) {
      var options = this.options;
      return Math.min(Math.max(options.minWidth, width), options.maxWidth);
    }
  };
